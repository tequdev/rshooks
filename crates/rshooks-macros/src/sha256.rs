//! Hand-written SHA-256 (FIPS 180-4), used by [`crate::base58`] to verify a
...
/// Computes the SHA-256 digest of `data` in a single shot, per FIPS 180-4.
// Every index/slice below is bounded by a fixed-size array (`K`/`H0`/`w`: 64
// or 8 elements, `out`: 32 bytes) and a loop range that never exceeds it —
// never data-length-dependent.
#[allow(clippy::indexing_slicing)]
pub(crate) fn sha256(data: &[u8]) -> [u8; 32] {
    // Padding (FIPS 180-4 §5.1.1): append a single `1` bit, then `0` bits,
    // then the 64-bit big-endian message length in bits, so the total
    // length is a multiple of 512 bits (64 bytes).
    ...
    for block in msg.chunks_exact(64) {
        ...
        // Compression function (FIPS 180-4 §6.2.2 step 3), 64 rounds.
