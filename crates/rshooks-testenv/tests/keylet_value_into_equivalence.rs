//! Equivalence check for `perf/keylet-into`'s delegation refactor: for
//! every one of the 26 typed keylet helpers in `rshooks::api::keylet`, the
//! by-value `keylet_xxx(...)` form and its `keylet_xxx_into(&mut out, ...)`
//! twin must compute the exact same `Result<Keylet>` for the same
//! arguments. Driven under an installed backend with **real** keylet
...
//!
//! Guards specifically against the delegation refactor (each by-value form
//! now zero-inits a local `Keylet` and calls its `_into` twin instead of
//! `util_keylet_buf` directly) silently threading a different argument to
//! the two paths — a plain "both compile" check would miss that; this
