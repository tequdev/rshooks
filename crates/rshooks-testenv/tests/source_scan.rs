...
//!    `keylet.rs`'s `util_keylet(` calls — see
...
/// `api/keylet.rs`'s 26 typed helpers each have an `_into` twin that
/// intercepts the backend with its own real slices before falling through
/// to `util_keylet` (a crate-local composing call, not a bare
/// `rshooks_core::<fn>(` one); the by-value form delegates to its `_into`
/// twin instead of calling `util_keylet` itself, so only the `_into`
/// functions carry a raw call site. [`find_raw_call`]'s `"rshooks_core::"`
/// needle never matches those calls, so this file's raw-call marker for
/// keylet.rs is a bare `util_keylet(` call — excluding a `.util_keylet(`
/// method call (the testenv-only backend dispatch inside `testenv_keylet`
/// itself, `b.util_keylet(...)`, not a raw host call site) by requiring the
/// character right before the needle not be an identifier character or
/// `.`.
