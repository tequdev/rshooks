...
once per keylet type using a small `check`/`store` helper pair — `check`
rolls back with `100 + keylet_type` on failure, `store` writes the already-
filled `keylet` to state). It reaches for `keylet_account_into` rather than
`keylet_account` precisely because `keylet` is about to be borrowed into
`state_set` right away — see "Why typed helpers" above. Every keylet here
is computed entirely from inputs already available at compile time or read
directly off the invoking transaction — no other ledger object has to
exist first, so the hook works against a bare node with no setup.
