...
| 21 | `txn-template-nested` | `txn_template!`'s fixed-shape nested containers: a Remit whose `sfAmounts` array is a homogeneous, indexed set of 48-byte issued-`amount` entries with a compile-time-baked currency/issuer, plus an `sfMemos` entry with a fixed-length `fixed_vl` memo type and data, filled through the `amounts(i)`/`memos(i)` accessors and emitted via `prepare_for_emit()`/`Prepared::emit()` | [Emitting Transactions](../emit/emitting.md) |
