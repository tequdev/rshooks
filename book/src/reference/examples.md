...
| 21 | `txn-template-nested` | `txn_template!`'s fixed-shape nested containers: a Remit whose `sfAmounts` array is a homogeneous, indexed set of 48-byte issued-`amount` entries with a compile-time-baked currency/issuer, filled through the `amounts(i)` accessor and emitted via `prepare_for_emit()`/`Prepared::emit()` | [Emitting Transactions](../emit/emitting.md) |
