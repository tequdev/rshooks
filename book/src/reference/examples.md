...
| 21 | `txn-template-nested` | `txn_template!`'s fixed-shape nested containers: a Remit whose `sfAmounts` array holds a native `sfAmountEntry` and a 48-byte issued-`amount` entry with a compile-time-baked currency/issuer, emitted via `prepare_for_emit()`/`Prepared::emit()` | [Emitting Transactions](../emit/emitting.md) |
