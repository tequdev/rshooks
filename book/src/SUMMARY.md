# Summary

[Introduction](introduction.md)

# Getting Started

- [Installation](getting-started/installation.md)
- [Your First Hook](getting-started/first-hook.md)
- [Building a Hook](getting-started/building.md)

# Core Concepts

- [Anatomy of a Hook](concepts/anatomy.md)
- [Hook Chains](concepts/chains.md)
- [Accept, Rollback, and Errors](concepts/errors.md)
- [Guards and Loops](concepts/guards.md)
- [Tracing and Debugging](concepts/tracing.md)

# Working with Data

- [Reading the Originating Transaction](data/otxn.md)
- [Hook State](data/state.md)
- [Hook and Transaction Parameters](data/parameters.md)
- [Typed Data with Derives](data/typed-data.md)
- [XFL: Decimal Floating Point](data/xfl.md)
- [Slots and Ledger Objects](data/slots.md)
- [Keylets](data/keylets.md)
- [Typed Views](data/views.md)

# Emitting Transactions

- [Emitting Transactions](emit/emitting.md)
- [The StoWriter API](emit/sto-writer.md)

# Build Toolchain

- [The rshooks CLI](build/cli.md)
- [Per-Hook Attributes and the SetHook Template](build/metadata.md)

# Testing

- [Off-Chain Unit Tests](testing/unit-tests.md)

# Reference

- [Macro Reference](reference/macros.md)
- [The Prelude](reference/prelude.md)
- [The Raw Layer](reference/raw.md)
- [Examples Index](reference/examples.md)
