...
| 22 | [`txn-template-optional`](22_txn-template-optional) | `txn_template!`'s NOP-padded optional and variable-length kinds (`docs/NOP_PADDING_DESIGN.md`), mostly through their inferred (`optional sfX`) spellings: `optional <kind>(sfX)`, `any_amount(sfX)`, `vl(sfX, MIN, MAX)`, a whole-container `optional object`/`optional array` view, a homogeneous array with `0` or `1` `optional` elements, and a named array with one required and one `optional` element — a Payment and two Remits (isolating `vl`'s `MAX`-proportional WCE in its own entry), every field legal for its transaction type per `crates/rshooks-core/protocol_formats.json`, with per-container 63-NOP budgets checked at compile time |
...
output prints the authoritative values). The take-once flag costs a few dozen bytes over a
raw `static mut` — the
