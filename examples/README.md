...
| 21 | [`txn-template-nested`](21_txn-template-nested) | `txn_template!`'s homogeneous indexed array form (`array(sfX) [ Elem: object(sfY) { .. } ; N ]`): a Remit whose two-element `sfAmounts` array (an issued `amount` entry with a baked currency/issuer, repeated) is declared once and filled at runtime through a `guard!`-bounded loop over a generated `amounts(index) -> Option<Elem<'_>>` accessor, with no `StoWriter` needed |
