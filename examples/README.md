...
| 22 | [`txn-template-optional`](22_txn-template-optional) | `txn_template!`'s NOP-padded optional field kinds (`docs/NOP_PADDING_DESIGN.md`), written entirely in the inferred style: `optional sfX`, `any_amount` via the `= AnyAmount()` default-shape marker, and a named array with one required and one `optional` element — a single Remit sending one or two amounts, with an optional `DestinationTag`, with per-container 63-NOP budgets checked at compile time |
