...
| 21 | [`txn-template-nested`](21_txn-template-nested) | `txn_template!`'s fixed-shape nested `object`/`array` fields: a Remit whose two-entry `sfAmounts` array (one `native_amount` entry, one `amount` entry with a baked currency/issuer) is declared entirely at compile time, with no `StoWriter` needed |
